...
参考性能（AMD Ryzen 7 4800H、Node 24、Windows 11，语料为 `test/cases.json`
20 句循环至约 100 万码点，预热后计时；对照为 Python 3.12 移植版 + Cython，
两边输出逐字节一致）：

| 指标 | 本实现 | Python 3.12（Cython） |
|---|---:|---:|
| 冷加载（分词 / +词性） | 142 / 180 ms | 4,310 / 4,834 ms |
| 稳态吞吐（分词，逐句调用） | 222,504 字符/秒 | 36,451 字符/秒 |
| 稳态吞吐（+词性，逐句调用） | 146,319 字符/秒 | 29,687 字符/秒 |
| 整段大文本（分词，单次调用） | 266,350 字符/秒 | 43,591 字符/秒 |
| 常驻内存（分词 / +词性） | 205 / 255 MiB | 1,203 / 1,272 MiB |

此前一次基准（Node.js 22、优化前的特征查询路径）：模型加载约 90–100 ms，
常驻内存约 212 MiB，吞吐约 2.8 万字符/秒。具体数值随硬件、语料重复度与
磁盘缓存状态变化。
